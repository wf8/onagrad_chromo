...
# add tip labels
p = ggtree(t, size=0.1) + geom_tiplab(align=TRUE, size=.75, linesize = 0.05) + ggplot2::xlim(0, 200) 
...
p = p + geom_nodepoint(aes(colour=end_state_1_pp, size=end_state_1), alpha=.15) +
...
ggsave("ancestral_states.pdf", width = 9, height = 9)
print(p)
